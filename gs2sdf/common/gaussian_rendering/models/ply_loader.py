...
Vendored from gs_sensors/gs_sensor_core/models/ply_loader.py.
"""
...
from gs2sdf.common.gaussian_rendering.compression import apply_compression, prune_low_opacity
...
def _sanitize(arr: np.ndarray, label: str) -> np.ndarray:
    ...
    if n_bad > 0:
        print(f"[gs2sdf] {label}: zeroing {n_bad:,} NaN/Inf values")
...
def load_gaussian_model(
    ...
    print(f"[gs2sdf] Loaded {model.num_points:,} splats (SH degree {degree}, "
