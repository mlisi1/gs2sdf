...
Vendored from gs_sensors/gs_sensor_core/models/gaussian_model.py.
"""
...
class GaussianModel:
    ...
    def render_fields(self, mask: torch.Tensor | None = None):
        ...
        rather than activating-then-indexing -- sigmoid/exp/normalize/cat
        are nontrivial per-point ops, so doing them over the full model and
        discarding most of the result makes their cost independent of how
        much culling actually removes."""
        if mask is None:
